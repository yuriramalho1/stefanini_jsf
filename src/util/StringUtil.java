...
public class StringUtil {
	...
	public static String formataMoeda(double valor, boolean moeda) {
		...
		if (valor >= 0) {
			resultado = new DecimalFormat("#,##0.00").format(valor);// NumberFormat.getCurrencyInstance().format(valor);
			...
			resultado = new DecimalFormat("#,##0.00").format(valor);
			//resultado = NumberFormat.getCurrencyInstance().format(valor);
			//resultado = "R$ -" + resultado.substring(4, resultado.length());
			if (moeda) {
				resultado = "R$ " + resultado;//resultado.substring(3, resultado.length());
			}
//			if (!moeda) {
//				resultado = resultado.substring(3, resultado.length());
//			}
